...
@section('content')
<section class="section-light bg-light shadow-sm">
    <div class="container">
        <div class="py-3">
                ...
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item" style="font-size:12px">
                        <a href="{{ route('web.home.index') }}">
                            <ion-icon name="home-sharp" class="m-0 p-0" style="font-size: 18px"></ion-icon>
                        </a>
                    </li>
                    <li class="breadcrumb-item active" style="font-size:12px" aria-current="page">
                        <a href="{{route('web.product.index')}}" class="txt-decoration-none">
                            <ion-icon name="cube-sharp" class="m-0 p-0" style="font-size: 18px"></ion-icon>
                        </a>
                    </li>
                    <li class="breadcrumb-item" style="font-size:12px" aria-current="page"><a
                            href="{{route('web.productCat.show', $product->product_cat->product_cat_slug)}}"
                            class="txt-decoration-none"><span>{{$product->product_cat->product_cat_name}}</span></a>
                    </li>
                    <li class="breadcrumb-item active" style="font-size:12px" aria-current="page"><a
                            href="{{url()->current()}}"
                            class="txt-decoration-none"><span>{{\Str::limit($product->product_name, 20)}}</span></a></li>
                </ol>
            </nav>
        </div>
    </div>
...
<section class="section-light py-3 mb-3">
    <div class="container">
        <div class="bg-light shadow-sm mb-2">
            <img src="{{url('engine/storage/app/product/300x300/'.$product->product_cover)}}" class="img-fluid" alt="{{ $product->product_name }}" width="500" height="500">
        ...
        <h1 class="fs-6">{{$product->product_name}}</h1>
        <span class="badge bg-secondary">12 Kali Dilihat</span>
        <p class="fs-2 my-2"><b>Rp.{{number_format($product->product_price)}}</b></p>
        <hr>
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link cl-dark-2" id="speasifikasi-tab" data-bs-toggle="tab"
                    data-bs-target="#spesifikasi" type="button" role="tab" aria-controls="profile"
        ...
        <div class="tab-content" id="myTabContent">
            <div class="tab-pane fade show active py-2" id="deskripsi" role="tabpanel"
                aria-labelledby="home-tab">
                <h2 class="fs-6">Jual {{$product->product_name}}</h2>
                <p style="font-size:12px;"><a class="txt-decoration-none" href="{{url()->current()}}">{{$product->product_name}}</a>
                    -
...
<div class="section-light py-3">
    <div class="container">
        <h3 class="fs-6">{{$product->product_cat->product_cat_name}} Lainnya</h3>
        <div class="row g-2">
                ...
                <div class="bg-light shadow-sm">
                    <a href="{{route('web.product.show', $v->product_slug)}}">
                        <img src="{{url('engine/storage/app/product/150x150/'.$v->product_cover)}}"
                            alt="{{$v->product_name}}" class="img-fluid" width="300" height="300">
                        <div class="p-2">
                            <h3 style="font-size:12px">{{$v->product_name}}</h3>
                            <p style="font-size:12px">SKU : {{$v->product_sku}}</p>
                            <p class="fs-3">Rp{{number_format($v->product_price, 0,",",".")}}</p>
                        </div>
                    </a>
                </div>
            </div>
